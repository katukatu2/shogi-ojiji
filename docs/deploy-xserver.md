...
エックスサーバーのサーバーパネル →「サブドメイン設定」で `shogi` を追加し、無料独自 SSL を有効にする。

ドキュメントルートは既定の **`/godo-amity.com/public_html/shogi/`** でよい。

**HTTPS は必須。** `SharedArrayBuffer` は安全なコンテキストでしか使えないため、http では動かず、エンジンが起動せずに「判定: 簡易」へ静かに落ちる。

#### 二重に見えることへの対処（重要）

エックスサーバーのサブドメインは**親ドメインの公開領域の下**に作られるので、同じ場所が 2 つの URL で見える。

```
https://shogi.godo-amity.com/     ← 意図した URL
https://godo-amity.com/shogi/     ← 親ドメイン経由でも見えてしまう
```

**別オリジンになるため、Service Worker・localStorage・キャッシュが二重に作られ、同じ人の成績が二か所に分かれる。** ドキュメントルートの名前を変えても解決しない。

`.htaccess` の「0.」の節が、正しいホストと HTTPS へ 301 で寄せてこれを塞ぐ。**サブドメイン名を変えるときは、この節の `shogi.godo-amity.com` も直すこと。**
...
- ホーム画面から起動したとき、対局画面の上の帯（対 ○○・判定など）が時計や電池の表示と重ならない（`tests/e2e/safe-area.spec.ts` が Chromium で模擬して確かめる。1.0.1 までは重なっていた）
