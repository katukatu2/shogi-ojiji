...
```
src/
  engine/   将棋のルール（盤面、合法手、反則、詰み、千日手・連続王手の千日手・入玉宣言（27 点法）・手数上限 400、SFEN、表記）
...
- 対局後は読ませる量を絞る。オジジの一言は一つだけ（勝ちは戦法ごとの台詞、ヒント 10 回以上なら「頼りすぎじゃ」、負け・投了・引き分けは戦法との戦い方の教え）。「ばかもん／悪手／ヒント／待った」は 0 でないものだけを一行で出す。免状と課題達成は文字の帯ではなく朱印にして顔の左右に押す（`src/ui/results.ts`。OS の視差効果を減らす設定では押す演出を省く）。課題を逃したときは「課題は次回」とだけ出す。長い振り返りは出さない。
...
- **称号と免状**: 戦法ごとに「初勝利」「叱られず勝利」「皆伝」（師範代でばかもん 0 かつ課題達成で勝つ）の免状が付く（`badgesOf`、`BADGE_LABEL`）。皆伝の数で称号が上がる（門前の小僧 → 通いの弟子 → 内弟子 → 免許皆伝、`titleOf`）。タイトルに称号と通算成績、対局設定の一覧に免状、結果画面に新しく付いた免状を朱印で出す（皆伝で称号が変わったときは称号も一行で出す）。
- **引き分け**: 千日手・400 手は引き分け（`GameResult` の `'draw'`）。局数にだけ数え、勝ちには数えない。連続王手の千日手は王手を続けた側の負け。
- **入玉宣言（27点法）**: 自分の手番で、自玉が敵陣3段以内・王手なし・敵陣に玉以外の自駒10枚以上・先手28点／後手27点以上なら宣言勝ち。点数は敵陣の自駒（玉以外）と持ち駒だけを、飛・角（成駒を含む）5点、他1点で数える。双方の入玉は不要。画面の「持将棋を宣言する」は成立時だけ押せ、操作しなければ点数不足で自動的に負けにならない。オジジは自分の手番で成立すれば宣言する。時間制限はないので残り時間の条件は設けない。[CSA大会規定 第23条](https://www.computer-shogi.org/wcsc23/rule.html)に基づく。24点法の合意による引き分けは採用しない。
...
- 不明なフラグ名とフラグの重複も、実行開始より前に拒否する。`--all-style` のような綴り違いを既定の矢倉へ読み替えない。引数は `--style yagura` のように値を別に渡す形式。
