// 製品UIを初期局面から操作する長い対局の確認。開発APIや保存データの注入は使わない。
// npx vite-node scripts/product-playthrough.ts -- --url http://127.0.0.1:5181 --out logs/product-playthrough
...
const out = arg('out', 'logs/product-playthrough');
...
try {
  await engine.init();
  await page.goto(arg('url', 'http://127.0.0.1:5181'));
  ...
  const limit = Number(arg('plies', '240'));
